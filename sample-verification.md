...
`scripts/verify-a4.mjs`. All of them pass at this commit; the integration
blocker this report originally raised against `src/audio/align.ts` has been
fixed — see the closed section at the end.
...
| the user’s note is re-anchored to the new transcript | ✓ 245.48 s → 250.48 s, `quality:"word"` |

All 20 of its assertions pass. The re-anchor was the one that did not, for a
reason outside this package; that cause is fixed — see the closed section below.

## ✅ Closed — integration blocker in `src/audio/align.ts` (WP1)

**Fixed in `ff2ee27`, with the dev fixture brought into line in `6401e69`.** The
diagnosis below is kept verbatim because it is the reason the fix exists; the
past tense is now the correct reading of it. `verify-sample` Layer 1b reports
17/17 quotes re-found at their exact span, `verify-a4` is 20/20, and
`verify-engine` Case E is 4/4 with the exact quote landing on words 60..74 at
ratio 1.0.

<details>
<summary>The original diagnosis, as filed</summary>
